...
| `ChatGPT - Codex` | `57` | enabled | OAuth local / sem `base_url` | `gpt-5.5`, `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.3-codex-spark` |
...
- OpenAI SDK local contra `http://127.0.0.1:3000/v1` com `gpt-5.5` via `ChatGPT - Codex`: OK com `ATIUS_ROUTER_STREAM=1`.
...
- `ChatGPT - Codex` esta ativo no channel 5 com modelos `gpt-5.5`, `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.3-codex-spark`.
...
## Codex CI / auth / release oficial

Para CI/auth/release do Codex neste fork:

- a fonte de verdade e `docs/CODEX-CI-AUTH-RELEASE.md`
- a referência externa autoritativa e a documentação oficial da OpenAI/Codex
- no GitHub Actions, preferir `openai/codex-action@v1`
- fora do GitHub Actions, preferir `codex exec`
- API key continua sendo o default para automação
- `auth.json` com `auth_mode: chatgpt` fica restrito a runner privado/confiável

Nao inventar refresh custom de token em jobs arbitrários quando o tema for
