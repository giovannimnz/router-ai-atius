...
- [ ] Phase 33: corrigir a semântica das métricas do reranker, readiness de routing/cache, timeout/retry TEI e persistência do fork-sync sem regredir o runtime governado.
...
The main checkout may be dirty with unrelated fork/runtime work. Phase 21 implementation must start from a clean worktree or branch based on current `upstream/main`. Phase 33 is incident-driven and preserves the live governed reranker path, channel 10, and both aliases in `EMBEDDING_GOVERNOR_MODELS`.
...
| Saude do reranker precisa separar outcome de request de disponibilidade do servico | O `88,89%` observado foi `8/9` por uma rejeicao local valida de payload, enquanto falhas de routing anteriores ao relay ficaram fora da metrica | Active in Phase 33 |
| Promocao do fork deve preservar o reranker governado completo | Uma imagem sem `tei_rerank.go`, canal 10 ou alias do governor remove capacidade live mesmo quando o build passa | Active in Phase 33 |

---
*Last updated: 2026-08-14 for Phase 33 reranker reliability planning.*
