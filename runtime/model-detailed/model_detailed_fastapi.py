...
DEEPSEEK_ANTHROPIC_MODELS = {"deepseek-v4-flash", "deepseek-v4-pro"}
MINIMAX_EMBEDDING_TYPES = {"query", "db"}
...
def enrich_models_response_anthropic(upstream_data: dict) -> dict:
    ...
    for ability in abilities:
        ...
        if model_name.startswith("deepseek") and model_name not in DEEPSEEK_ANTHROPIC_MODELS:
            continue
...
def enrich_openai_models_response_anthropic(openai_data: dict) -> dict:
    ...
    for model in openai_data.get("data", []):
        ...
        if not model_name or model_name in models_seen:
            continue

        endpoint_types = model.get("supported_endpoint_types", [])
        if "anthropic" not in endpoint_types:
            if model_name.startswith("deepseek") and model_name not in DEEPSEEK_ANTHROPIC_MODELS:
                continue
            if not model_name.startswith("MiniMax") and model_name not in DEEPSEEK_ANTHROPIC_MODELS:
                continue
        if model_name.startswith("deepseek") and model_name not in DEEPSEEK_ANTHROPIC_MODELS:
            continue
    ...
    for model in upstream_data.get("data", []):
        ...
        if model_id == "embo-01":
            enriched.append({
                "id": model_id,
                "object": "model",
                "created": model.get("created", MODEL_CREATED_TS),
                "owned_by": "minimax",
...
async def health_check():
    ...
    client = await get_http_client()
    try:
        resp = await client.get(f"{BACKEND_URL}/api/status", timeout=5)
        if resp.status_code == 200:
            return {"status": "healthy", "backend": "connected"}

        # Fallback for NewAPI builds that do not expose /api/status. /v1/models
        # is authenticated, so 401 still proves the backend is reachable.
        models_resp = await client.get(f"{BACKEND_URL}/v1/models", timeout=5)
        if models_resp.status_code in {200, 401}:
            return {"status": "healthy", "backend": "connected"}
    ...
    try:
        if api_format == "anthropic":
            ...
            try:
                ...
                # Fall back to the authenticated OpenAI-format model list and
                # return the Anthropic-capable subset instead of failing with 500.
...
def _proxy_request_headers(request: Request) -> dict:
    """Forward request headers that are safe for httpx to resend."""
    headers = {}
...
def _normalise_embedding_input(payload: dict) -> tuple[dict, bool]:
    """
    MiniMax embo-01 requires `texts` as an array. NewAPI's embeddings DTO only
    accepts OpenAI `input`, so keep the request OpenAI-shaped and normalize a
    scalar string into a single-item array before channel param_override copies
    it to `texts`.
    """
    if payload.get("model") != "embo-01":
        return payload, False

    body = dict(payload)
    body["type"] = body.get("type") if body.get("type") in MINIMAX_EMBEDDING_TYPES else "query"
...
def _openai_embedding_response(payload: dict, upstream_data: dict) -> dict | None:
    vectors = upstream_data.get("vectors")
    ...
    total_tokens = upstream_data.get("total_tokens")
    ...
    return {
        ...
        "model": upstream_data.get("model") or payload.get("model", "embo-01"),
        "usage": {
...
def _embedding_error_status(upstream_data: dict) -> int:
    base_resp = upstream_data.get("base_resp")
...
async def create_embeddings(request: Request):
    """
    OpenAI-compatible embeddings adapter for MiniMax embo-01.

    The Go backend routes/authenticates/logs the request. This middleware only
    normalizes OpenAI input shape before forwarding and converts MiniMax's
    native vectors response back to OpenAI embedding format.
    """
    client = await get_http_client()
    ...
    try:
        ...
        if isinstance(parsed, dict):
            payload, should_normalise = _normalise_embedding_input(parsed)
            if should_normalise:
