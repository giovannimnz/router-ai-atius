...
# ==============================================================================
# The session cookie is generated by new-api's gin-contrib/sessions middleware
# (gorilla/securecookie format: timestamp|base64(gob_values)|hmac). The cookie name
# is "session" and is HttpOnly + Secure + SameSite=Strict when emitted by new-api.
#
# Validation strategy: decode the cookie using the same SESSION_SECRET as new-api
# (passed via env to both containers) to extract the user_id. Then call new-api's
# /api/user/self endpoint with BOTH the session cookie AND the New-Api-User header
# (which new-api's middleware/auth.go requires since commit rc.2).
#
# The decode is done by a small Go helper binary (/usr/local/bin/decode-cookie) that
# uses gin-contrib/sessions directly — guaranteed format compatibility.
# ==============================================================================

DECODE_COOKIE_BIN = "/usr/local/bin/decode-cookie"


def _decode_session_via_helper(cookie_value: str) -> dict | None:
    """Call the Go helper to decode the gorilla/securecookie session cookie.

    Returns a dict with keys: user_id, username, role, status, group, ok.
    Returns None on decode failure.
    """
    import subprocess
    try:
        result = subprocess.run(
            [DECODE_COOKIE_BIN, cookie_value],
            capture_output=True,
            text=True,
            timeout=2.0,
        )
...
async def validate_session_cookie(session_cookie: str | None) -> bool:
    """Validate session cookie by:
    1. Decoding it to extract user_id (via Go helper, same SESSION_SECRET as new-api)
    2. Calling new-api /api/user/self with both the session cookie AND the
       New-Api-User header (new-api requires this header since rc.2).
    """
    if not session_cookie:
        return False

    # Step 1: Decode the cookie to get user_id
    session_data = _decode_session_via_helper(session_cookie)
    if not session_data:
        return False
    ...
    # Step 2: Forward cookie + New-Api-User header to new-api for validation
