...
- Local TEI embeddings and reranking are governed inside the Go router through `service/embeddinggovernor/`, `relay/embedding_handler.go`, and `relay/rerank_handler.go`. Do not reintroduce Python/model-detailed, a sidecar, or an extra container for these paths. The default public governed models are `embedding-gte-v1` and `reranker-gte-multilingual-v1`; preserve both in `EMBEDDING_GOVERNOR_MODELS`. Batch is selected by request metadata, not by a public `*-batch` alias. Daily concurrency must keep fallback `min=1` and start at `initial=2`, but no longer uses a static router ceiling: `EMBEDDING_GOVERNOR_MAX_CONCURRENCY=0` and `EMBEDDING_GOVERNOR_BATCH_CONCURRENCY=0` mean unbounded by static cap, with scale controlled by governor feedback, queue limits, cooldown, health, latency, and read-only capacity telemetry from the TEI pods. Capacity usage at or above `EMBEDDING_GOVERNOR_CAPACITY_MAX_USED_PERCENT` (default `80`) must block scale-up; consecutive bad capacity windows may reduce concurrency. Keep the TEI embedding input-array cap of `4` and reranker document cap of `20` unless transparent sub-batching/recomposition is implemented. Preserve the Advanced Custom `jina_rerank_to_tei_native` converter and its public `documents`/`results[].relevance_score` contract.
- Runtime directories must stay out of build context through `.dockerignore`: `/backups`, `/data`, `/logs`, `/runtime`.
- Valid personal tokens must never be rejected because user, wallet, account, or subscription balance is exhausted or negative. Subscription absence/exhaustion always falls through to wallet accounting, including `subscription_only` and `allow_wallet_overflow=false`; only explicit token limits and upstream provider limits may block. Preserve `scripts/atius-user-quota-guard.sh`, `patches/atius-user-quota-unlimited.patch`, and `docs/ATIUS-USER-QUOTA-INVARIANT.md` as the build, sync, recovery, and release-readback contract for this invariant.

When updating fork-sync protection, keep these paths protected or manually port their semantics before merging upstream changes: `.dockerignore`, `controller/model.go`, `controller/model_list_test.go`, `service/modelcatalog/`, `relay/common/relay_utils.go`, `relay/common/relay_utils_test.go`, `relay/embedding_handler.go`, `relay/embedding_handler_test.go`, `relay/rerank_handler.go`, `relay/channel/advancedcustom/`, `relay/channel/codex/`, `service/codex_*.go`, `service/embeddinggovernor/`, `k8s/router-ai-atius/configmap.yaml`, `service/pre_consume_quota.go`, `service/billing_session.go`, `service/quota.go`, `relay/mjproxy_handler.go`, `service/billing_session_wallet_overdraft_test.go`, `scripts/atius-user-quota-guard.sh`, `patches/atius-user-quota-unlimited.patch`, `docs/`, and `.planning/`.
