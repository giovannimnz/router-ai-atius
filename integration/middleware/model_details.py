...
for DeepSeek models, while transparently proxying all other requests to
the NewAPI backend.

Usage:
    python3 model_enrichment.py [--port PORT] [--backend BACKEND_URL]
...
def enrich_models_response(upstream_data):
    """Enrich the /v1/models response with DeepSeek metadata."""
    enriched = []
    for model in upstream_data.get("data", []):
        model_id = model.get("id", "")
        metadata = DEEPSEEK_METADATA.get(model_id)
...
def main():
    ...
    print(f"[middleware] Enriching models: {', '.join(DEEPSEEK_METADATA.keys())}")
