...
- Valid personal tokens must never be rejected because user, wallet, account, or subscription balance is exhausted or negative. Subscription absence/exhaustion always falls through to wallet accounting, including `subscription_only` and `allow_wallet_overflow=false`; only explicit token limits and upstream provider limits may block. Preserve `scripts/atius-user-quota-guard.sh` and `patches/atius-user-quota-unlimited.patch` as the build/sync gate for this invariant.

When updating fork-sync protection, keep these paths protected or manually port their semantics before merging upstream changes: `.dockerignore`, `controller/model.go`, `controller/model_list_test.go`, `service/modelcatalog/`, `relay/common/relay_utils.go`, `relay/common/relay_utils_test.go`, `relay/embedding_handler.go`, `relay/channel/codex/`, `service/codex_*.go`, `service/embeddinggovernor/`, `service/pre_consume_quota.go`, `service/billing_session.go`, `service/quota.go`, `relay/mjproxy_handler.go`, `scripts/atius-user-quota-guard.sh`, `patches/atius-user-quota-unlimited.patch`, `docs/`, and `.planning/`.
