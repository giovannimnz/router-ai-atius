...
func SystemOneHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	sysOneReq, ok := info.Request.(*dto.SystemOneRequest)
	if !ok {
	...
	request, err := common.DeepCopy(sysOneReq)
	if err != nil {
	...
	if model_setting.GetGlobalSettings().PassThroughRequestEnabled || info.ChannelSetting.PassThroughBodyEnabled {
		...
		var convertedRequest any = request
		if sysAdaptor, ok := adaptor.(SystemOneAdaptor); ok {
			convertedRequest, err = sysAdaptor.ConvertSystemOneRequest(c, info, request)
			if err != nil {
